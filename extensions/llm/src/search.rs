...
impl Extension for LlmSearch {
    ...
    async fn query(&self, query: &Query) -> Vec<ResultItem> {
        ...
        // A friendly subtitle that hints at the model when one is configured.
        let subtitle = match crate::load_config().active_provider() {
            Some(p) if !p.model.is_empty() => format!("Ask {} \u{2014} \u{201c}{}\u{201d}", p.model, query.text),
            _ => format!("Ask the AI \u{2014} \u{201c}{}\u{201d}", query.text),
        };

        vec![ResultItem {
            id: "llm-ask".to_string(),
            title: "Chat with AI".to_string(),
